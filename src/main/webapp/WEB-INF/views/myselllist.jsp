	...
	<table class="table table-bordered table-hover">
		...
		<c:forEach var="list" items="${tl}">
			<tr>
				...
				<td>${list.writer}</td>
				<td>${list.discount}</td>
					<td><button class="btn btn-info" onclick="mylistdel(this.name,${pb.currentPage},'sell')" name="${list.no }">삭제</button></td>
	...
	<div align="center">
		<ul class="pagination">
			<c:if test="${pb.startPage > pb.pagePerBlock }">
				<li><a onclick="mytradelist(1,'sell')">
						<span class="glyphicon glyphicon-step-backward"></span>
				</a></li>
				<li><li><a onclick="mytradelist(${pb.startPage-1},'sell')">${i}
						<span class="glyphicon glyphicon-triangle-left"></span>
				</a></li>
			</c:if>
			<c:forEach var="i" begin="${pb.startPage}" end="${pb.endPage}">
				<c:if test="${i!=pb.currentPage }">
					<li><a onclick="mytradelist(${i},'sell')">${i}</a></li>
				</c:if>
				<c:if test="${i==pb.currentPage }">
					<li class="active"><a onclick="mytradelist(${i},'sell')">${i}</a></li>
				</c:if>
			</c:forEach>
			<c:if test="${pb.endPage < pb.totalPage }">
				<li>
					<a onclick="mytradelist(${pb.endPage+1},'sell')">
					
					
						<span class="glyphicon glyphicon-triangle-right"></span>
				</a></li>
			</c:if>
			<li><a onclick="mytradelist(${pb.totalPage},'sell')">

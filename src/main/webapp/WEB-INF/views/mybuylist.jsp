...
<body>
	<h2 class="text-primary">내 구매글</h2>
	<h3 style="position:absoulute;border:none;border:1px double #337ab7"></h3>
	<form id="buyinsert" method="post" onsubmit="return false;">
	<input type="hidden" name="loc" value="mybuy">
	...
	<ul class="nav nav-tabs">
		<li class="nav-item"><a class="nav-link active" href="#" id="buylistbtn">구매글
				목록</a></li>
		<li class="nav-item"><a class="nav-link active" href="#" id="mybuylistbtn">내
				구매글 </a></li>
	</ul>
	...
	<table class="table table-bordered table-hover">
		...
		<c:forEach var="list" items="${tl}">
			<tr>
				...
				<td>${list.writer}</td>
				<td>${list.discount}</td>
						<td><button class="btn btn-info" onclick="mylistdel(this.name,${pb.currentPage},'buy')" name="${list.no }">삭제</button></td>
	...
	<div align="center">
		<ul class="pagination">
			<c:if test="${pb.startPage > pb.pagePerBlock }">
				<li><a onclick="mytradelist(1,'buy')">
						<span class="glyphicon glyphicon-step-backward"></span>
				</a></li>
				<li><li><a onclick="mytradelist(${pb.startPage-1},'buy')">${i}
						<span class="glyphicon glyphicon-triangle-left"></span>
				</a></li>
			</c:if>
			<c:forEach var="i" begin="${pb.startPage}" end="${pb.endPage}">
				<c:if test="${i!=pb.currentPage }">
					<li><a onclick="mytradelist(${i},'buy')">${i}</a></li>
				</c:if>
				<c:if test="${i==pb.currentPage }">
					<li class="active"><a onclick="mytradelist(${i},'buy')">${i}</a></li>
				</c:if>
			</c:forEach>
			<c:if test="${pb.endPage < pb.totalPage }">
				<li>
					<a onclick="mytradelist(${pb.endPage+1},'buy')">
					
					
						<span class="glyphicon glyphicon-triangle-right"></span>
				</a></li>
			</c:if>
			<li><a onclick="mytradelist(${pb.totalPage},'buy')">
	...
	<form id='searchform' method='post' onsubmit="return false;">
		...
		<input type="hidden" name="tag" value="buy">

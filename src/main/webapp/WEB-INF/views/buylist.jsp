	...
	<table class="table table-bordered table-hover">
		...
		<c:forEach var="list" items="${tl}">
			<tr>
				...
				<td>${list.writer}</td>
				<td>${list.discount}</td>
				<td><button class="btn btn-info" id="buyrequest">판매신청</button></td>
			</tr>
		</c:forEach>
	</table>
	<div align="center">
		<ul class="pagination">
			<c:if test="${pb.startPage > pb.pagePerBlock }">
				<li><a onclick="tradelist(1,'buy')">
						<span class="glyphicon glyphicon-step-backward"></span>
				</a></li>
				<li><li><a onclick="tradelist(${pb.startPage-1},'buy')">${i}
						<span class="glyphicon glyphicon-triangle-left"></span>
				</a></li>
			</c:if>
			<c:forEach var="i" begin="${pb.startPage}" end="${pb.endPage}">
				<c:if test="${i!=pb.currentPage }">
					<li><a onclick="tradelist(${i},'buy')">${i}</a></li>
				</c:if>
				<c:if test="${i==pb.currentPage }">
					<li class="active"><a onclick="tradelist(${i},'buy')">${i}</a></li>
				</c:if>
			</c:forEach>
			<c:if test="${pb.endPage < pb.totalPage }">
				<li>
					<a onclick="tradelist(${pb.endPage+1},'buy')">
					
					
						<span class="glyphicon glyphicon-triangle-right"></span>
				</a></li>
			</c:if>
			<li><a onclick="tradelist(${pb.totalPage},'buy')">
